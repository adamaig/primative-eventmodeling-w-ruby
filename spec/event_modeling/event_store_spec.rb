...
RSpec.describe EventModeling::EventStore do
  describe '#initialize' do
    let(:event_store) { described_class.new }

    it 'initializes with empty streams storage' do
      expect(event_store.instance_variable_get(:@streams)).to eq({})
    end

    it 'initializes with empty subscriptions storage' do
      expect(event_store.instance_variable_get(:@subscriptions)).to eq({})
    end

    it 'initializes with empty snapshots storage' do
      expect(event_store.instance_variable_get(:@snapshots)).to eq({})
    end

    it 'does not require lazy initialization of subscriptions' do
      # Test that @subscriptions is immediately available without ||= pattern
      stream_id = 'test-stream'
      callback = ->(event) { puts event }

      event_store.subscribe_to_stream(stream_id, callback)

      subscriptions = event_store.instance_variable_get(:@subscriptions)
      expect(subscriptions).to have_key(stream_id)
      expect(subscriptions[stream_id]).to include(callback)
    end

    it 'does not require lazy initialization of snapshots' do
      # Test that @snapshots is immediately available without ||= pattern
      stream_id = 'test-stream'
      snapshot_data = { user_id: 123, name: 'John' }
      version = 1

      event_store.save_snapshot(stream_id, snapshot_data, version)

      snapshots = event_store.instance_variable_get(:@snapshots)
      expect(snapshots).to have_key(stream_id)
      expect(snapshots[stream_id][:data]).to eq(snapshot_data)
    end

    it 'creates independent instances' do
      event_store1 = described_class.new
      event_store2 = described_class.new
  ...
  describe 'instance variable access patterns' do
    let(:event_store) { described_class.new }

    context 'with subscriptions' do
      it 'works without conditional assignment' do
        # This tests that the new initialization eliminates the need for @subscriptions ||= {}
        stream_id = 'test-stream'

        # Directly access @subscriptions without triggering ||= in subscribe_to_stream
        subscriptions = event_store.instance_variable_get(:@subscriptions)
        expect(subscriptions).to be_a(Hash)
        expect(subscriptions).to be_empty

        # Should work immediately without any ||= patterns
        subscriptions[stream_id] = []
        expect(subscriptions[stream_id]).to eq([])
      end
    end

    context 'with snapshots' do
      it 'works without conditional assignment' do
        # This tests that the new initialization eliminates the need for @snapshots ||= {}
        snapshots = event_store.instance_variable_get(:@snapshots)
        expect(snapshots).to be_a(Hash)
        expect(snapshots).to be_empty

        # Should work immediately without any ||= patterns
        snapshots['test'] = { data: 'test', version: 1 }
        expect(snapshots['test']).to eq({ data: 'test', version: 1 })
      end
    end
  end
